#!/usr/bin/env python3
"""
Improved clustering evaluation with MiniBatchKMeans.
...
def evaluate_k_values():
    ...
    embeddings_file = "temp/pubmedqa/corpus_embeddings.npy"
    ...
    logger.info(f"Sampled {sample_size:,} embeddings for evaluation.")
    
    # 测试的 k 值
    k_values = [1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000]
    ...
    # 开始评估
    for i, k in enumerate(tqdm(k_values, desc="Evaluating k values")):
        logger.info(f"Starting k={k} ({i+1}/{len(k_values)})")
    ...
    # === 保存结果 JSON ===
    json_path = "clustering_evaluation_results.json"
    ...
    plt.plot(valid_ks, valid_inertias, 'bo-', linewidth=2, markersize=8)
